...
document.addEventListener("keydown",function(e){if(e.code == "KeyI" && tokenArray.length != 0){if(document.getElementById("infopanel").style.display == "none"){openInfoPanel()}else{if(document.getElementById("infopanel2").style.display != "none"){document.getElementById("infopanel2").style.display = "none"}else{document.getElementById("infopanel").style.display = "none"}}} if((e.key == "+" || e.code == "KeyB") && document.getElementById("propertydialogue").style.display != "none"){buyProperty()};if((e.key == "-" || e.code == "KeyP") && document.getElementById("propertydialogue").style.display != "none"){passProperty()};if(e.code == "Space"){e.preventDefault();if(document.getElementById("chancedialogue").style.display != "none"){document.getElementById("closebtn").click()}};if(e.code == "Space" && !document.getElementById("rollbtn").disabled && document.getElementById("rollbtn").style.display != "none"){if(document.getElementById("rollbtn").innerText == "Roll dice"){roll()}else{endTurn()}}});
...
tokenArray[i].payoff=tmp[i].payoff;
}
...
if(i % 2 == 0 && !isNaN(Number(document.querySelector("form").querySelectorAll("input")[i].value[0]))){
isInvalid=true;
}
}
if(document.querySelector("form").checkValidity() && isUnique && !isWrongColor && !isInvalid){
if(localStorage.monopolyPropertyArray){
if(confirm("You still have a game in your local save slot. Would you like to download a save of this game as a file (to select click OK)? If not, your game saved locally will disappear forever.")){
tokenArray=new Array(2);
downloadSave();
tokenArray=[];
}
}
for(var i=0;i<players;i++){
	tokenArray.push(new Token(capitalize(document.querySelector("form").querySelectorAll("input")[i*2].value),(document.querySelector("form").querySelectorAll("input")[i*2+1].value).toLowerCase(),"Tokens/"+document.querySelector("form").querySelectorAll("select")[i].value+".png"))
}
...
document.getElementById("playermoney"+(i+1)).querySelector("img").src=tokenArray[i].src;
document.getElementById("playermoney"+(i+1)).style.display="block";
tokenArray[i].quadrant=i+1;
}
game.announce(tokenArray[0].name + " goes first.",function(){startTurn()});
goSalary=300-tokenArray.length*50;
document.getElementById("init").remove();
document.getElementById("main").style.display="block";
...
if(document.querySelector("form").checkValidity() && isUnique && isInvalid){
alert("Player names must not begin with a number.");
}
