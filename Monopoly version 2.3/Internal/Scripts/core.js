...
function propEvent(){
	for(var i=0;i<propertyArray.length;i++){
		...
		if(propertyArray[i].cost <= cp().money || propertyArray[i].ownedBy != "bank"){
			if(propertyArray[i].ownedBy == "bank"){
				...
				if(propertyArray[i].houses<5 && cp().money>=propertyArray[i].houseprice){
					...
					if(!propertyArray[i].isInMonopoly){rentArr=Math.floor(rentArr/2)}
...
}else{document.querySelector("textarea").innerHTML += cp().name+" does not have sufficient funds to purchase "+propertyArray[i].name+'\n'}
...
for(var i=0;i<tokenArray.length;i++){
document.getElementById("tokens").innerHTML += "<button a class='infobutton' style='width:100%;border:2px solid black' onclick=displayInformation(this)>"+tokenArray[i].name+"</button><br>"}

}
function displayInformation(a){
prop=game.getElementByName(a.innerHTML);
document.getElementById("infopanel2").style.display="block";
totalPayment=0;
propPayment=0;
houseNo=0;
for(var i=0;i<prop.propertiesOwned.length;i++){
totalPayment += Math.floor(prop.propertiesOwned[i].cost);
propPayment += Math.floor(prop.propertiesOwned[i].cost);
if(prop.propertiesOwned[i].className == "Property"){
totalPayment += Math.floor(prop.propertiesOwned[i].houses*prop.propertiesOwned[i].houseprice);houseNo += prop.propertiesOwned[i].houses}
}
document.getElementById("tokenpanelname").innerHTML = prop.name+" stats:";
document.getElementById("Characteristics").innerHTML = "Token: "+translateToTokenName(prop.src)+"<br>Current square: "+prop.square+", "+getSquareName(prop.square)+"<br>Money: $"+prop.money+"<br>Properties owned: "+prop.propertiesOwned.map(function(a){return a.name}).join(", ")+" ("+prop.propertiesOwned.length+" total, $"+propPayment+" total cost)<br>Investment: $"+totalPayment+"<br>Money gained through properties: $"+prop.payoff+"<br>Total number of houses: "+String(houseNo)+" ($"+String(totalPayment-propPayment)+" total cost)"
}
function getSquareName(num){
if(num == 0){
return "GO"}
if(num == 4){return "Income Tax"}
if(num == 2 || num == 17 || num == 33){return "Community Chest"}
if(num == 38){return "Luxury Tax"}
if(num == 7 || num == 22 || num == 36){return "Chance"}
if(num == 10){return "Jail, just visiting"}
if(num == 20){return "Free Parking"}
if(num == 30){return "Go to Jail"}
for(var i=0;i<propertyArray.length;i++){
if(propertyArray[i].square == num){
return propertyArray[i].name}}
return "Invalid square"
}
function translateToTokenName(src){
	str=src.split("/")[src.split("/").length-1].split(".png").join("");
	if(str == "Car"){return "Racecar"}
	if(str == "Hat"){return "Top Hat"}
	return str;
}