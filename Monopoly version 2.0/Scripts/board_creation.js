...
a.style.left="195px";
a.style.backgroundImage="url(gotojail.svg)";
}
if(i==0){
...
document.body.appendChild(barProp);
		...
		saveinterval=setInterval(game.save,1000)}},true)}
console.log(tmp);
console.log(i);
i=0;
...
for(var i=0;i<players;i++){
	tokenArray.push(new Token(document.querySelector("form").querySelectorAll("input")[i*2].value,document.querySelector("form").querySelectorAll("input")[i*2+1].value,"https://html5.gamemonetize.co/rb9b7ctnmy1rjphxzuwlmy8c4b7mw9po/"+document.querySelector("form").querySelectorAll("select")[i].value+".png"))
}
for(var i=0;i<tokenArray.length;i++){
document.getElementById("playermoney"+(i+1)).style.backgroundColor=tokenArray[i].color;
document.getElementById("playermoney"+(i+1)).querySelector("img").src=tokenArray[i].src;
...
document.body.appendChild(barProp);
		...
		startTurn();
		goSalary=300-tokenArray.length*50;
		document.getElementById("rollbtn").disabled=undefined}},true)}
console.log(tmp);
console.log(i);
i=0;
var tmp1=structuredClone(propertyArray);
