...
console.log(id)}
		...
		this.getCurrentRent=function(){
			...
			}else{return Math.floor(this.monopolyRentArray[this.houses]/2)}
		}
		this.houses=0;
...
this.houses++;
game.announce(cp().name+" bought a house on "+this.name+" for $"+this.houseprice,function(){});
		}else{
			var houses=document.getElementsByClassName("house"+this.square);
			console.log(houses);
			houses[1].style.width="50px";
			houses[1].style.backgroundColor="rgb(120,0,0)";
			if(this.square>10 && this.square<20){
			houses[1].style.left="160px";
			houses[1].style.bottom=((-1005+110.4*(this.square-11)))+"px";
			}
			if(this.square>30 && this.square<40){
			houses[1].style.left="1187px";
			houses[1].style.bottom=((-1005+110.4*(39-this.square)))+"px";
			}
			houses[0].remove();
			houses[1].remove();
			houses[1].remove();
			this.houses++;
			game.announce(cp().name+" bought a hotel on "+this.name+" for $"+this.houseprice);
	}}
...
document.querySelector("textarea").innerHTML += cp().name+" drew the Community Chest card \""+this.cardContent.split("<br>").join("\n")+"\"\n"
...
this.act=act;
this.action=function(){
document.querySelector("textarea").innerHTML += cp().name+" drew the Chance card \""+this.cardContent.split("<br>").join("\n")+"\"\n"
document.getElementById("chancedialogue").style.display="inline-block";
...
document.getElementById("chancedialogue").style.display="none";
document.getElementById("rollbtn").style.display="block";
if(act != undefined){cp().prevSquare=cp().square;act()};
}}}}
